...
use imageproc::{
    ...
    morphology::{Mask, grayscale_open},
};
...
pub trait ImageProc {
    ...
    line: PolarLine,
    image_width: u32,
    image_height: u32,) -> Option<((f32, f32), (f32, f32))>;
}
...
impl ImageProc for Reciept {
    ...
    fn crop_gray(&mut self) {
        ...
        let a = Self::intersection_points(*vertical_lines.last().unwrap(),self.image.width(),self.image.height()).unwrap();
        let b = Self::intersection_points(*vertical_lines.first().unwrap(),self.image.width(),self.image.height()).unwrap();
        let right = cmp::max(a.0.0.round() as u64, a.1.0.round() as u64);
        let left = cmp::min(b.0.0.round() as u64, b.1.0.round() as u64);
        let height = self.image.height();

        
        let answer = imageops::crop(&mut self.image, left as u32, 0, (right-left) as u32, height);
